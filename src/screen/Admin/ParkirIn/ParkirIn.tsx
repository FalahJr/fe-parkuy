...
import { parkirInService } from "../../../services";
...
export default () => {
	...
	const [parkirIn, setParkirIn] = useState<Array<ParkirInResponse>>([])
	const [detail, setDetail] = useState<DetailParkirInReponse>()
	const [showModal, setShowModal] = useState<boolean>(false)


	// Get All Data Location
	const getParkirIn = async () => {
		try {
			const response = await parkirInService.getParkirIn()
			setParkirIn(response.data)
	...
	const openModal = async (id: string) => {
		...
		try {
			const response = await parkirInService.detailParkirIn(id)
	...
	const columnsPosts: TableColumn<ParkirInResponse>[] = [
		{
			title: "No",
			key: "id_location",
			dataType: "numbering"
		...
		},
		{
			...
			{new Date(value.waktu_masuk).getMinutes()  }
			</div>
			// new Date(value.waktu_masuk).getMinutes() }
		},
		{
	...
	return (
		<>
			...
			/>
			<div className="table-content parking-in">
				<div className="table-container">
					...
					<Table columns={columnsPosts} data={parkirIn} />
				</div>
			</div>
			{/* {showModal ? ( */}
			<Modal visible={showModal} title="Data Parkir Masuk" onCancel={() => setShowModal(false)}>
				<form action="">
